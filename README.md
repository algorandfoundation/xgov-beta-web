...
For local development, the recommended path is to run `pnpm run mock-init`, which will bootstrap local configuration (including generating a `.env.development` based on the template) and deploy/initialize the xGov Registry on LocalNet.
...
pnpm run mock-init -- --council-address YOUR_ALGORAND_ADDRESS
```

Or using the short alias:

```bash
pnpm run mock-init -- -c YOUR_ALGORAND_ADDRESS
```
...
| `pnpm mock-init` | Initialize local development environment |
| `pnpm mock-init-assign` | Initialize with mock voter assignment data |
