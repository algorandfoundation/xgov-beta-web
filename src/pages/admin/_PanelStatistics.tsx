...
import { useAllXGovs } from "src/hooks/useRegistry";
    ...
    <>
      ...
      <div className="text-3xl text-wrap lg:text-3xl max-w-3xl text-algo-black dark:text-white font-bold mt-8 mb-4">
        <div className="pl-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {xGovs.data && xGovs.data.map((xGov, index) => (
            <div key={index} className="p-4 border rounded-lg shadow-md bg-white dark:bg-algo-black overflow-hidden">
      ...
      <div className="text-3xl text-wrap lg:text-3xl max-w-3xl text-algo-black dark:text-white font-bold mt-8 mb-4">
        <div className="pl-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {proposals.data && proposals.data.map((proposal) => (
            <div key={proposal.id.toString()} className="p-4 border rounded-lg shadow-md bg-white dark:bg-algo-black">
              <Link to={`/proposal/${proposal.id}`} className="text-black-500 hover:underline">
                <h3 className="text-xl font-semibold">{proposal.title}</h3>
              </Link>
              <p className={`text-sm ${proposal.status === ProposalStatus.ProposalStatusBlocked ? 'text-red-500' : 'text-gray-500'} dark:${proposal.status === ProposalStatus.ProposalStatusBlocked ? 'text-red-400' : 'text-gray-400'}`}>
