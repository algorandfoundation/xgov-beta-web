...
import type { CommitteeCohort } from "@/types/voting_cohort";
...
import { cidFromFile, cidStringToUInt8Array } from "@/functions/cid";
...
export function VotingCohort() {
  ...
  const [votingCohortNumber, setVotingCohortNumber] = useState<number | null>(null);
  const [votingCohortVotePower, setVotingCohortVotePower] = useState<number | null>(null);
  ...
  return (
    <div>
      ...
      <div className="relative bg-white dark:bg-algo-black border-2 border-algo-black dark:border-white text-algo-black dark:text-white p-4 rounded-lg max-w-3xl">
        ...
        {fileCID && (
          <div className="mt-4">
            <p><strong>CID V1:</strong> {fileCID}</p>
          </div>
        )}
        {votingCohortNumber !== null && (
          <div className="mt-4">
            <p><strong>Number of Members:</strong> {votingCohortNumber}</p>
          </div>
        )}
        {votingCohortVotePower !== null && (
          <div className="mt-4">
            <p><strong>Total Vote Power:</strong> {votingCohortVotePower}</p>
          </div>
        )}
