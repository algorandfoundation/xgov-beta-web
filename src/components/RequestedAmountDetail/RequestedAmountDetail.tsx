...
const detailVariants = cva("inline-flex items-center text-xs md:text-lg", {
  variants: {
    variant: {
...
const tokenIconVariants = cva("stroke-[6] size-5 ml-1 mr-3", {
  variants: {
    variant: {
...
const algoIconVariants = cva("size-3 md:size-4 ml-1", {
  variants: {
    variant: {
...
export interface RequestedAmountDetailProps
  extends VariantProps<typeof detailVariants> {
  requestedAmount: bigint;
}

export function RequestedAmountDetail({
  variant,
  requestedAmount,
}: RequestedAmountDetailProps) {
  return (
