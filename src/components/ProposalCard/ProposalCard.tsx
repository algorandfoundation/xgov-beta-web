import { FundingCategoryMap, isProposalInfoCardDetails, isProposalSummaryCardDetails, ProposalFundingCategory, ProposalFundingTypeMap, statusToPhase, type ProposalCardDetails, type ProposalInfoCardDetails, type ProposalMainCardDetails, type ProposalSummaryCardDetails } from "@/types/proposals";
...
function ProposalSummaryCard({
    ...
    return (
        <li role="listitem" className="list-none relative bg-white dark:bg-algo-black border-2 border-algo-black dark:border-white text-algo-black dark:text-white p-4 rounded-lg max-w-3xl">
            <h3 className="text-lg text-wrap w-52 md:w-[20rem] lg:max-w-[38rem] lg:text-2xl mt-10 mb-6 lg:mb-14 font-bold">{title}</h3>
            <p className="text-xl">{FundingCategoryMap[category]}</p>
            <p className="text-xl">{ProposalFundingTypeMap[fundingType]}</p>
            <p className="text-xl mb-6 lg:mb-14">{(Number(requestedAmount) / 1_000_000).toLocaleString()} ALGO</p>
            ...
            <div className="absolute top-0 right-0 mt-4 mr-4 flex flex-col items-end gap-4">
                ...
                <p className="text-lg my-4 mr-2">- {proposer.length === 58 ? shortenAddress(proposer) : proposer }</p>
            </div>
    ...
    return (
        <li role="listitem" className="list-none relative bg-white dark:bg-algo-black border-2 border-algo-black dark:border-white text-algo-black dark:text-white p-4 rounded-lg max-w-xl lg:min-w-[36rem]">
            <div className="max-w-3xl">
                ...
                <p className="text-xl font-normal dark:text-algo-blue-10">{fundingType}</p>
